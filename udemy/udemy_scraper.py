...
from courses import ScrapCourses_link
...
class UdemyScrap:
    ...
    def scrap_courses(self):
        ...
        file_link = exists('courses_links.csv')
        if file_link:
            d = pd.read_csv('courses_links.csv', index_col=0)
            all_links = list(d['links'])
        else:
            all_links = ScrapCourses_link().get_courses_link()
